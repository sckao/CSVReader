...
// fileNme : csv file name , data vector : store readout, nSkipLine: number of line to skip for possible header
void AnaInput::ReadFreeCSV( string fileName, vector<vec>& data, int nSkipLine, int nStopLine ) {
  ...
  while ( getline( logfile, line) ) {
        ...
        ii++ ;
        if ( ii <= nSkipLine ) continue ;
        if ( nStopLine > 0 && ii > nStopLine ) break ;
...
void AnaInput::ReadStringCSV( string fileName, vector<svec>& data, int nSkipLine, int nStopLine ) {
  ...
  while ( getline( logfile, line) ) {
	...
	for (string::iterator it = line.begin(); it< line.end(); it++) {
            ...
            if ( (*it) == ',' || it == line.end()-1 ) {
               if ( vtemp.size() < 1 ) continue ;
	       if (debug == 1)  printf("%s ", vtemp.c_str() ) ;
	       rowV.push_back( vtemp ) ;
	       vtemp.clear() ;
            }
        }
	if (debug == 1) printf("\n" ) ;
